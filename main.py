...
st.set_page_config(
    page_title="Advanced Keyword Tagging Tool (v3)",
    page_icon="🏷️🎯📊",
    layout="wide",
...
import nltk
import shutil
import openai # Kept for potential future OpenAI embedding use, but not active now
...
try:
    ...
    # st.sidebar.success("NLTK components initialized.") # Keep sidebar less cluttered
except Exception as e_init: st.error(f"Failed to initialize NLTK: {e_init}. App cannot continue."); st.stop()

# --- Global Stopwords (can be customized via UI) ---
CUSTOMIZABLE_STOP_WORDS = set(STOP_WORDS_ENGLISH) # Base
DEFAULT_DOMAIN_BC_STOPWORDS = { # Default suggestions for the UI
    "style", "type", "series", "version", "model", "standard", "item",
    "product", "new", "best", "top", "custom", "design", "feature", "collection",
    "buy", "price", "cost", "for sale", "near me", "online", "cheap" # Common query modifiers
}
    ...
    with st.spinner(f"Getting OpenAI embeddings with {model} ({len(processed_texts)} texts)..."):
        ...
        for i in range(0, len(processed_texts), batch_size_items):
            ...
            try:
                ...
                st.error(f"OpenAI API error: {e_openai}")
                pass 
...
def get_pos_filtered_normalized_tokens(text_segment, user_a_tags_for_bc_filter):
    ...
    for token, tag in tokens_with_pos:
        if tag.startswith('NN') or tag.startswith('JJ') or tag == 'CD': # Added CD for cardinal numbers
            normalized_token = normalize_token(token)
            # Use st.session_state.current_custom_bc_stopwords for filtering
            if normalized_token not in st.session_state.current_custom_bc_stopwords and \
               (len(normalized_token) > 0): # Allow single digit numbers after normalization
...
def classify_keyword_maximized(keyword_str, seed_to_remove_str, other_omitted_list_str, user_a_tags_set_runtime, kw_model_runtime):
    ...
    temp_text_after_a_tag_removal = original_kw_lower
    for a_tag_cand in sorted_user_a_tags:
        match = re.search(rf'\b{re.escape(a_tag_cand)}\b', temp_text_after_a_tag_removal, flags=re.IGNORECASE)
        if match:
            identified_a_tag = a_tag_cand 
            temp_text_after_a_tag_removal = re.sub(rf'\b{re.escape(a_tag_cand)}\b', '', temp_text_after_a_tag_removal, count=1, flags=re.IGNORECASE).strip()
            break 
    text_for_bc_processing = ' '.join(temp_text_after_a_tag_removal.split())
    ...
    # For this check, filter out B/C stopwords BEFORE checking if it's a lone common product term
    ...
    if identified_a_tag != "general-other" and \
       ...
       normalized_remaining_words_for_check[0] in common_product_terms:
        # If only one word remains, it's a common product term, and NOT a B/C stopword, assign to B
        b_tag = normalized_remaining_words_for_check[0]
        return identified_a_tag, b_tag, "" # C is empty

    if not text_for_bc_processing: return identified_a_tag, "", "" 

    keyphrases = kw_model_runtime.extract_keywords(text_for_bc_processing, keyphrase_ngram_range=(1,3), stop_words=st.session_state.current_custom_bc_stopwords, top_n=1)
...
def run_semantic_clustering(keywords_list, embedding_model_name_or_path, cluster_accuracy_percent, min_cluster_size_val, 
    ...
    # ... (run_semantic_clustering function remains the same) ...
    ...
    # st.write("Performing community detection...")
    ...
    for i, cluster in enumerate(clusters):
        cluster_name_temp = f"Semantic Cluster {i + 1}" 
...
openai_api_key_input = st.sidebar.text_input("OpenAI API Key (for optional semantic clustering)", type="password", key="openai_api_key_sidebar_main")
msv_column_name = st.sidebar.text_input("Column Name for MSV in your file", "Monthly Searches", key="msv_col_name_sidebar_main")
...
with col1_tag_cfg:
    seed_input = st.text_input("Primary Seed Keyword to Remove (optional)", "", key="tag_seed_main_v3")
    omit_str_input = st.text_input("Other Phrases to Omit (comma-separated)", "buy,price,cost,for sale,near me,online,cheap,best,top", key="tag_omit_main_v3")
with col2_tag_cfg:
    user_atags_str_input = st.text_input("Define Primary Categories (A:Tags, comma-separated)", "patio door,patio doors,door,window,cabinet,panel,shaker,flat,raised,wood,metal,glass,interior,exterior,kitchen,bathroom", key="tag_a_tags_main_v3") # Added patio door(s)
    do_realign_input = st.checkbox("Re-align B/C tags by frequency", value=True, key="tag_realign_main_v3")
...
if st.sidebar.button("Update B/C Stopwords", key="update_bc_stopwords"):
    if custom_bc_stopwords_ui_input:
        st.session_state.current_custom_bc_stopwords = set(STOP_WORDS_ENGLISH) # Start with base
...
with st.expander("Optional: Initial Tagging Rules (CSV Upload)"):
    # ... (Initial rule UI remains the same) ...
    initial_rule_file_input = st.file_uploader("Upload Rule CSV", type=["csv"], key="tag_rule_file_main_v3")
    use_initial_rule_input = st.checkbox("Use Initial Tagging Rule if available", value=False, key="use_tag_rule_main_v3")
...
uploaded_file_main = st.file_uploader("Upload Keyword File (CSV/Excel - must include 'Keywords' and MSV column)", type=["csv", "xls", "xlsx"], key="main_file_uploader_v3")
...
if uploaded_file_main is not None and not st.session_state.get('full_tagging_processed', False):
    with st.expander("Optional: Pre-Analysis - Semantic Keyword Clustering to Inform Tagging Settings", expanded=False):
        # ... (Semantic Pre-Analysis UI remains the same) ...
        ...
        with col_sem1: sem_cluster_accuracy = st.slider("Semantic Similarity Threshold (%)", 0, 100, 80, key="sem_accuracy_v3")
        with col_sem2: sem_min_cluster_size = st.number_input("Min Keywords per Semantic Cluster", 2, 20, 3, key="sem_min_size_v3")

        if st.button("Run Semantic Pre-Analysis", key="run_semantic_pre_analysis_v3"):
    ...
    if st.session_state.get('semantic_clusters_generated', False) and 'df_semantic_clusters_preview' in st.session_state:
        ...
        # ... (Display logic for semantic pre-analysis results remains the same) ...
        ...
        if not df_sem_preview.empty:
            ...
            st.download_button("Download Full Semantic Pre-Analysis CSV", df_sem_preview.to_csv(index=False).encode('utf-8'), "semantic_pre_analysis_clusters.csv", "text/csv", key="dl_sem_pre_analysis_v3")
...
if 'full_tagging_processed' in st.session_state and st.session_state.full_tagging_processed:
    if st.button("Re-Process File with Current Settings", key="reset_tagging_main_reprocess_v3"): 
...
if not st.session_state.get('full_tagging_processed', False):
    if uploaded_file_main is not None: 
        if st.button("Run Full A/B/C Tagging Process", key="run_tagging_main_button_v3"):
...
if st.session_state.get('full_tagging_processed', False):
    ...
    # ... (Display and download logic for tagging results remains the same) ...
    ...
    if not df_display.empty:
        ...
        try:
            ...
            st.download_button(label="Download Full Tagged Keywords CSV (with MSV)", data=csv_tagged, file_name="tagged_keywords_msv_final.csv", mime="text/csv", key="dl_full_tags_btn_msv_final_v3")
    ...
    if not summary_ab_display.empty:
        ...
        try:
            ...
            st.download_button(label="Download Tag Summary CSV (with MSV)", data=csv_summary, file_name="tag_summary_msv_final.csv", mime="text/csv", key="dl_tag_summary_btn_msv_final_v3")
