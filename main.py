...
def extract_candidate_tags(candidate):
    ...
    Given a candidate theme (a string), use POS-tagging to recommend:
      ...
      - C:Tag: all tokens before the chosen noun.
      - D:Tag: all tokens after the chosen noun.
...
st.markdown(
    ...
    **Workflow:**
    ...
    3. Specify how many keyphrases to extract per keyword.
...
if uploaded_file:
    ...
    if "Keywords" not in df.columns:
        ...
        # Normalize candidate themes so that singular/plural variants are grouped.
        normalized_themes = {}
        for phrase in all_phrases:
            norm_phrase = normalize_phrase(phrase)
            if norm_phrase:
                normalized_themes[norm_phrase] = normalized_themes.get(norm_phrase, 0) + 1
        
        # Filter candidate themes by minimum frequency.
        candidate_themes = {theme: freq for theme, freq in normalized_themes.items() if freq >= min_freq}
